...
//        public UBAHordeSession(Uri hordeUri, AuthenticationHeaderValue? authHeader, string? pool, bool allowWine, int maxCores, bool strict, ConnectionMode? connectionMode, Encryption? encryption, ILogger logger)
//        {
...
//            _client = new ServerComputeClient(_serviceProvider.GetRequiredService<IHttpClientFactory>(), null, _logger);
...
//        public static async Task<UBAHordeSession?> TryCreateHordeSession(bool bStrictErrors, UbaSettings ubaSettings, ILogger logger, CancellationToken cancellationToken = default)
//        {
//            string? server = ubaSettings.UbaHordeConfig.HordeServer;
//            string? token = ubaSettings.UbaHordeConfig.HordeToken;
//            string? oidcProvider = ubaSettings.UbaHordeConfig.HordeOidcProvider;
//            string? hordePool = ubaSettings.UbaHordeConfig.HordePool;
//            string? hordeConnectionMode = ubaSettings.UbaHordeConfig.HordeConnectionMode;
//            string? hordeEncryption = ubaSettings.UbaHordeConfig.HordeEncryption;
//            string? hordeCondition = ubaSettings.UbaHordeConfig.HordeCondition;
//            string? ubaSentryUrl = ubaSettings.UbaHordeConfig.UBASentryUrl;
//            bool hordeAllowWine = ubaSettings.UbaHordeConfig.bHordeAllowWine;
//            int hordeMaxCores = ubaSettings.UbaHordeConfig.HordeMaxCores;
...
//            ConnectionMode? connectionMode = Enum.TryParse(hordeConnectionMode, true, out ConnectionMode cm) ? cm : null;
//            Encryption? encryption = Enum.TryParse(hordeEncryption, true, out Encryption enc) ? enc : null;
...
//                server, hordePool ?? "(none)", hordeCondition ?? "(none)", hasOidcProvider ? oidcProvider! : "Disabled", connectionMode?.ToString() ?? "(none)", encryption?.ToString() ?? "(none)");
//            try
//            {
...
//                bool allowWine = hordeAllowWine && OperatingSystem.IsWindows();

//                UBAHordeSession session = new(new Uri(server), authHeader, hordePool, allowWine, hordeMaxCores, bStrictErrors, connectionMode, encryption, logger);
//                await session.InitAsync(useSentry: !String.IsNullOrEmpty(ubaSentryUrl), cancellationToken);
//                return session;
//            }
...
//        async Task RunWorkerAsync(Worker self, IComputeLease lease, UbaSessionServer server, UbaSettings settings, BlobLocator tool, string executable, ILogger logger, CancellationToken cancellationToken)
//        {
...
//                        string hordeHost = settings.UbaHordeConfig.HordeHost;
//                        int port = settings.UbaConfig.Port;

//                        bool useListen = !String.IsNullOrEmpty(hordeHost);
//                        List<string> arguments = new();

//                        arguments.Add($"-Host={hordeHost}:{port}");
...
//                        arguments.Add("-Quiet");
//                        string? ubaSentryUrl = null;
//                        if (!String.IsNullOrEmpty(ubaSentryUrl))
//                        {
//                            arguments.Add($"-Sentry=\"{ubaSentryUrl}\"");
//                        }
...
//                                server!.AddClient(self.Ip, self.Port.Port, _crypto);
//                                shouldConnect = false;
//                            }
//                        }
